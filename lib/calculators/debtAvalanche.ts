...
export interface AvalancheInputs {
  debts: DebtItem[];
  extraPayment: number;
}
...
export const calculateAvalanche = (inputs: AvalancheInputs): AvalancheResult => {
  // Sort debts by interest rate (highest to lowest)
  const sortedDebts = [...inputs.debts].sort((a, b) => b.interestRate - a.interestRate);
  ...
  while (debtBalances.some(balance => balance > 0)) {
    ...
    // Find the first active debt (highest interest rate)
